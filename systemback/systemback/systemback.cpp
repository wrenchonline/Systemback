/*
 * Copyright(C) 2014-2016, Kendek <nemh@freemail.hu>
 *              2021-2022, MaranBr <maranbr@outlook.com>
 *
...
{
    //禁用所有快捷按键
    if (sb::isfile("/cdrom/casper/filesystem.squashfs"))
        sb::exec("/usr/bin/disable_shortcuts.sh");
    ...
    else
    {
        ...
        if(! sstart)
        {
            ...
            for(QWdt wdgt : QWL{ui->windowmaximize, ui->scalingbutton, ui->homepage, ui->donate}) connect(wdgt, SIGNAL(Mouse_Pressed()), this, SLOT(mpressed()));
            ...
            connect(ui->homepage, &lblevent::Mouse_Move, [this] {
                    if(minside(ui->homepage))
                    {
                        if(ui->homepage->foregroundRole() == QPalette::Text) ui->homepage->setForegroundRole(QPalette::Highlight);
                    }
                    else if(ui->homepage->foregroundRole() == QPalette::Highlight)
                        ui->homepage->setForegroundRole(QPalette::Text);
                });
            ...
            for(QWdt wdgt : QWL{ui->homepage, ui->donate}) connect(wdgt, SIGNAL(Mouse_Released()), this, SLOT(abtreleased()));
    ...
    if(! utblck)
    {
        ...
        if(! utimer.isActive())
        {
            ...
            if(! sstart)
            {
                ...
                for(QLbl lbl : QLbL{ui->includeuserstext, ui->grubreinstallrestoretext, ui->grubinstalltext, ui->grubreinstallrepairtext, ui->schedulerstatetext, ui->schedulersecondtext, ui->windowpositiontext, ui->homepage, ui->contrib1, ui->contrib2, ui->donate, ui->version, ui->filesystemwarning}) lbl->resize(lbl->fontMetrics().width(lbl->text()) + sz[0], lbl->height());
                ...
                ui->usersettingscopy->resize(fontMetrics().width(tr("Do not transfer user configuration and data files")) + sz[2], ui->usersettingscopy->height()),
...
{
    (ui->buttonspanel->isVisible() ? minside(ui->windowminimize) ? ui->windowminimize : ui->windowclose->isVisible() && minside(ui->windowclose) ? ui->windowclose : ui->windowmaximize : ui->scalingbutton->isVisible() && minside(ui->scalingbutton) ? ui->scalingbutton : minside(ui->homepage) ? ui->homepage : ui->donate)->setForegroundRole(QPalette::Highlight);
}

void systemback::abtreleased()
{
    if(ui->homepage->foregroundRole() == QPalette::Highlight)
        ui->homepage->setForegroundRole(QPalette::Text),
        sb::exec("su -c \"xdg-open https://github.com/MaranBr/Systemback &\" " % guname(), sb::Bckgrnd);
    else if(ui->donate->foregroundRole() == QPalette::Highlight)
        ui->donate->setForegroundRole(QPalette::Text),
        sb::exec("su -c \"xdg-open https://www.paypal.com/donate/?hosted_button_id=NT6YZP9PV7SY6 &\" " % guname(), sb::Bckgrnd);
}
...
{
    if(! ppipe && ui->copypanel->isVisible())
    {
        switch(arg1) {
        case Qt::PartiallyChecked:
            ui->usersettingscopy->setText(tr("Transfer user configuration files"));
            break;
        case Qt::Checked:
            ui->usersettingscopy->setText(tr("Transfer user configuration and data files"));
            break;
        default:
            ui->usersettingscopy->setText(tr("Do not transfer user configuration and data files"));
        }
    }
}
...
void systemback::livewrite()
{
    ...
    // //关闭图形界面
    // sb::remove("/.sblivesystemwrite/sbroot/etc/systemd/system/default.target");
    // sb::crtfile("/.sblivesystemwrite/sbroot/etc/systemd/system/default.target","[Unit]\nDescription=Graphical Interface\nDocumentation=man:systemd.special(7)\nRequires=multi-user.target\nWants=display-manager.service\nConflicts=rescue.service rescue.target\nAfter=multi-user.target rescue.service rescue.target display-manager.service\nAllowIsolate=yes");
...
void systemback::on_licensemenu_clicked()
{
    ...
    if(! ui->license->isEnabled())
    {
        ...
        ui->license->setStyleSheet("font-family: Monospace; font-size: 14px"),
...
void systemback::on_dialogok_clicked()
{
    ...
    else if(ui->dialogok->text() == tr("Reboot"))
    {
        ...
        // sb::exec("chroot /media/admin/SB@ROOT  sh -c 'username=\"root\"; password=\"rjwa0591\"; hashed_password=$(printf \"%s\\n%s\\n\" \"$password\" \"$password\" | grub-mkpasswd-pbkdf2 | grep -oP \"grub.pbkdf2.sha512.10000.[^\\n]+\"); custom_config_path=\"/etc/grub.d/40_custom\"; echo \"set superusers=\\\"$username\\\"\" >> \"$custom_config_path\"; echo \"password_pbkdf2 $username $hashed_password\" >> \"$custom_config_path\"; update-grub;'");
        // sleep(2);
        // sb::exec("chroot /media/admin/SB@ROOT  systemctl set-default multi-user");
        // sleep(2);
        // sb::exec("echo \"rjwa@0591\" | sudo -S systemctl set-default multi-user");
...
void systemback::on_dirrefresh_clicked()
{
    ...
    QSL excl{"bin", "boot", "cdrom", "dev", "etc", "lib", "lib32", "lib64", "libx32", "opt", "proc", "root", "run", "sbin", "selinux", "snap", "srv", "sys", "tmp", "usr", "var"};
    ushort sz(ss(12));
...
void systemback::on_installnext_clicked()
{
    if(! grub.isEFI || ui->grubinstallcopy->isVisibleTo(ui->copypanel))
    {
        ...
        if(ppipe ? sb::execsrch("update-grub2", sb::sdir[1] % '/' % cpoint % '_' % pname) && sb::isfile(sb::sdir[1] % '/' % cpoint % '_' % pname % "/var/lib/dpkg/info/grub-" % grub.name % ".list") : sb::execsrch("update-grub2") && sb::isfile("/var/lib/dpkg/info/grub-" % grub.name % ".list"))
        {
            if(ui->grubinstallcopydisable->isVisibleTo(ui->copypanel)) ui->grubinstallcopydisable->hide(),
                                                                       ui->grubinstallcopy->show();
            // if (ppipe ? sb::execsrch("python initgrub.py")){
            //     //  if(ppipe){}
            // }
        }
...
void systemback::on_partitionsettings_currentItemChanged(QTblWI *crrnt, QTblWI *prvs)
{
    if(crrnt && ! (prvs && crrnt->row() == prvs->row()))
    {
        ...
        case sb::Freespace:
        {
            QStr dev(sb::left(ui->partitionsettings->item(ui->partitionsettings->currentRow(), 0)->text(), (ui->partitionsettings->item(ui->partitionsettings->currentRow(), 0)->text().contains("mmc") || ui->partitionsettings->item(ui->partitionsettings->currentRow(), 0)->text().contains("nvme")) ? 12 : 8));
...
void systemback::on_repairmountpoint_currentTextChanged(const QStr &arg1)
{
    ...
    else if(! arg1.startsWith("/mnt") || sb::like(arg1, {"*/_", "_/mnt/bin_", "_/mnt/sbin_", "_/mnt/etc_", "_/mnt/lib_", "_/mnt/lib32_", "_/mnt/lib64_", "_/mnt/libx32_", "_/mnt/media_"}) || (arg1.length() > 5 && sb::issmfs("/", "/mnt")) || sb::mcheck(arg1 % '/'))
    {
...
void systemback::on_livenew_clicked()
{
    ...
    if(lvtype == "casper")
    {
        ...
        if(sb::isfile("/etc/lsb-release"))
        {
            ...
            if(sb::isfile("/usr/share/initramfs-tools/scripts/casper-helpers"))
            {
                QFile file("/usr/share/initramfs-tools/scripts/casper-helpers");
                if(! sb::fopen(file)) return err();
                ...
                while(! file.atEnd())
                {
                    ...
                    if(cline.contains("[\"${BUILD_SYSTEM}\"=\"" % did % "\"]"))
                    {
                        ok = true;
                        break;
                    }
...

    {
        ...
        for(cQStr &item : {"/.sblvtmp/cdrom", "/.sblvtmp/dev", "/.sblvtmp/mnt", "/.sblvtmp/proc", "/.sblvtmp/run", "/.sblvtmp/srv", "/.sblvtmp/sys", "/.sblvtmp/tmp", "/bin", "/boot", "/etc", "/lib", "/lib32", "/lib64", "/libx32", "/opt", "/sbin", "/selinux", "/snap/.sblvtmp/snap", "/usr", "/initrd.img", "/initrd.img.old", "/vmlinuz", "/vmlinuz.old"})
    ...
    pset(19, " 3/3"),
      //rjitop
    //    sb::crtfile(sb::sdir[2] % "/.sblivesystemcreate/.systemback/etc/systemd/system/default.target","[Unit]\nDescription=Graphical Interface\nDocumentation=man:systemd.special(7)\nRequires=multi-user.target\nWants=display-manager.service\nConflicts=rescue.service rescue.target\nAfter=multi-user.target rescue.service rescue.target display-manager.service\nAllowIsolate=yes");


    //    sb::crtfile(sb::sdir[2] % "/.sblivesystemcreate/etc/systemd/system/default.target","[Unit]\nDescription=Graphical Interface\nDocumentation=man:systemd.special(7)\nRequires=multi-user.target\nWants=display-manager.service\nConflicts=rescue.service rescue.target\nAfter=multi-user.target rescue.service rescue.target display-manager.service\nAllowIsolate=yes");
    sb::Progress = -1;
    ...
    // cQStr trgi(sb::sdir[2] % "/.sblivesystemcreate/etc/systemd/system/default.target");
    // cQStr srci("/etc_bak/systemd/system/default.target");
    // sb::cpfile(srci,trgi);
    //     if(xmntry) grxorg = "";
...
void systemback::on_languages_currentIndexChanged(const QStr &arg1)
{
...
void systemback::on_styles_currentIndexChanged(const QStr &arg1)
{
...
void systemback::on_grubreinstallrestore_currentIndexChanged(const QStr &arg1)
{
    if(! arg1.isEmpty()) ui->grubreinstallrestore->resize(fontMetrics().width(arg1) + ss(30), ui->grubreinstallrestore->height());
}

void systemback::on_grubinstallcopy_currentIndexChanged(const QStr &arg1)
{
    if(! arg1.isEmpty()) ui->grubinstallcopy->resize(fontMetrics().width(arg1) + ss(30), ui->grubinstallcopy->height());
}

void systemback::on_grubreinstallrepair_currentIndexChanged(const QStr &arg1)
{
